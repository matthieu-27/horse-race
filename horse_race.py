...
def display_result(_horse_list):
    ...
    for horse in _horse_list:
        progress = horse[1] // 30
        progress_bar = '▓' * progress + '░' * (2400 // 30 - progress)
        if count < len(_horse_list):
            if horse[1] == "DQ":
                print(f"\033[91m{HORSE_STR + str(count + 1)} - {horse[1]} : {progress_bar}\033[0m")
            elif horse[1] >= MAX_DISTANCE and winners < game_mode:
                print(f"\033[92m{HORSE_STR + str(count + 1)} - {horse[1]} : {progress_bar}\033[0m")
                winners += 1
            else:
                print(f'{HORSE_STR + str(count + 1)} - {horse[1]} : {progress_bar}')
            count += 1
...
def check_speed(_turn_speed, _random_number):
    """ Returns True if not disqualified, False otherwise """
    if _turn_speed == 3 and _random_number == 6:
        return False
    return True
...
if __name__ == "__main__":
    ...
    while not game_over:
        ...
        for i in range(len(horses)):
            ...
            if check_speed(turn_speed, random_number):
                horses[i][0] = turn_speed
                horses[i][1] += get_turn_distance(turn_speed)
            else:
                disqualified.append(horses[i])
                if len(disqualified) >= MIN_HORSES:
                    game_over = True
                    break
